...
- 하단 탭바(4개: 홈/위클리/숏폼/설정), 카드 피드, 좌우 스와이프로 이전·다음 호 이동
...
| 경로 | 역할 |
|---|---|
| `/` | 홈 피드 (Weekly + Shortform 최신 섞어서 카드 리스트) |
| `/weekly` | 위클리 탭 (전체 주간 리스트) |
| `/weekly/[week]` | 개별 주간 페이지. 좌우 스와이프로 전/다음 호 이동 |
| `/shortform` | 숏폼 탭 |
| `/shortform/[week]` | 개별 숏폼 페이지 |
